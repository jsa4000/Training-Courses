...
for i=1:num_neg_examples
    ...
    x = this_case'; %Hint
    activation = this_case*w; % This evaluate the activation 
    if (activation >= 0)
        %YOUR C ODE HERE
        % If activation is greater or equal to 0 means this weights is not corrrect.
        % We need to update. Awi = xi (t - y). Since it's a matrix I can evaluate
        % all weight at the same time
        w = w + epsilon_lr * x * (0 - activation)
    end
end
...
for i=1:num_pos_examples
    ...
    if (activation < 0)
        %YOUR CODE HERE
        % As linear this could be less that 0. Target is 1 so this fragement of code
        % it's in fact incorrect.
         w = w + epsilon_lr * x * (1 - activation)
    end
end
