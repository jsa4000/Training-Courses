...
def get_bow_train(inpath, ngrams=1, use_tfidf=False):
    ...
    if ngrams>1:
        # First fit the data to get the vocabulary
        vectorizer.fit(df["text"])  
        #Trransfrom the data using previosu vocabulary
        if use_tfidf:
    ...
    return bag, df["polarity"].astype(int).values, vectorizer.vocabulary_
...
def get_bow_test(inpath, ngrams=1, vocabulary=None, use_tfidf=False):
    ...
    # Split the text column with the ngrams and find the occurrences
    if use_tfidf:
        vectorizer = TfidfVectorizer(vocabulary=vocabulary,ngram_range=(ngrams,ngrams))
    else:
        vectorizer = CountVectorizer(vocabulary=vocabulary,ngram_range=(ngrams,ngrams))
    # Extract the words and get the vectors
    if ngrams>1:
        # First fit the data to get the vocabulary
        vectorizer.fit(df["text"])  
...
def predict_sentiment_analyis(inpath, testpath, ngrams=1, use_tfidf=False):
    ...
    gd = GridSearchCV(model, parameters, cv=list(kf.split(train, labels)))
    gd.fit(train, labels)
